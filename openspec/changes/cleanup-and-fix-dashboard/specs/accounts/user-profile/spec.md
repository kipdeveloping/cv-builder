...
The system SHALL maintain a UserProfile linked one-to-one with each User, containing a photo field, bio text field, is_public boolean, and professional title. The profile SHALL use SectorTag foreign key for sector classification. The `sector_name` field is REMOVED.
...
- **AND** if `headline` is empty, `title` is synced to `headline`
