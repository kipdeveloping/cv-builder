document.addEventListener('DOMContentLoaded', function() {
    ...
    async function saveResume() {
        ...
        isSaving = true;
        saveStatus.textContent = t('Guardando...');
        ...
        try {
            ...
            if (result.status === 'ok') {
                saveStatus.textContent = `${t('Guardado a las')} ${result.timestamp}`;
                saveStatus.className = 'text-sm text-green-600';
            } else {
                saveStatus.textContent = t('Error al guardar');
                saveStatus.className = 'text-sm text-red-600';
            }
        } catch (error) {
            console.error('Error saving:', error);
            saveStatus.textContent = t('Error de conexión');
            saveStatus.className = 'text-sm text-red-600';
        } finally {
            isSaving = false;
        }
            ...
            if (section === 'experience') {
                html = `
                    <div class="entry" data-section="experience" data-index="${index}">
                        <div class="entry-title" contenteditable="true" data-field="title">${t('Título del puesto')}</div>
                        <div class="entry-subtitle" contenteditable="true" data-field="company">${t('Empresa')}</div>
                        <div class="entry-period" contenteditable="true" data-field="period">${t('2020 - Presente')}</div>
                        <div class="entry-description" contenteditable="true" data-field="description">${t('Descripción...')}</div>
                    </div>
                `;
            } else if (section === 'education') {
                html = `
                    <div class="entry" data-section="education" data-index="${index}">
                        <div class="entry-title" contenteditable="true" data-field="degree">${t('Título')}</div>
                        <div class="entry-subtitle" contenteditable="true" data-field="school">${t('Institución')}</div>
                        <div class="entry-period" contenteditable="true" data-field="year">${t('2018')}</div>
                    </div>
                `;
            }
    ...
    if (addSkillBtn) {
        addSkillBtn.addEventListener('click', function() {
            ...
            newSkill.textContent = t('Nueva habilidad');
