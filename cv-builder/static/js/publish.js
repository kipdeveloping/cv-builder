document.addEventListener('DOMContentLoaded', function() {
    ...
    async function publishResume() {
        ...
        if (tags.length === 0) {
            publishError.textContent = t('Debes seleccionar al menos un sector.');
            publishError.classList.remove('hidden');
            return;
        }

        if (!primaryTag || !tags.includes(primaryTag)) {
            publishError.textContent = t('Debes seleccionar un sector principal válido.');
            publishError.classList.remove('hidden');
            return;
        }

        confirmPublish.disabled = true;
        confirmPublish.textContent = t('Publicando...');
        ...
        try {
            ...
            if (result.status === 'ok') {
                ...
                publishError.textContent = result.message || t('Error al publicar.');
                publishError.classList.remove('hidden');
            }
        } catch (error) {
            console.error('Error publishing:', error);
            publishError.textContent = t('Error de conexión. Inténtalo de nuevo.');
            publishError.classList.remove('hidden');
        } finally {
            confirmPublish.disabled = false;
            confirmPublish.textContent = t('Confirmar publicación');
        }
    }
